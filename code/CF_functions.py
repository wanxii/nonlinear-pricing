...
def Consumer_matrix(V_it, Qbar_it, smooth=False, b=4, m=0.48525429746950965):
    Consumers = np.zeros((len(V_it), 501)) 
    # break tie toward larger quantity given same WTP
    noise = np.array(range(501)) * 1e-5
    if not smooth: 
        for i in range(len(V_it)):
            Consumers[i,:] = Consumer_i(v_it=V_it[i], qbar_it=Qbar_it[i], q=np.linspace(0, 500, 501)) + noise
    else:
        for i in range(len(V_it)):
            Consumers[i,:] = Smooth_Consumer_i(v_it=V_it[i], qbar_it=Qbar_it[i], q=np.linspace(0, 500, 501), b=b, m=m) + noise
...
def Profit(Y, Q, cmat, cost, continuity, group=None):
    sched = Schedule(Y=Y, Q=Q, q=np.linspace(1, 500, 500), continuity=continuity) 
